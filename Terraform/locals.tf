locals {
  ...
  loogic_storage_account_name = format("lasa%sfa%s%s%s", "azureintegration", var.env, "sea", var.instance)

  # Service Bus
  servicebus_namespace = format("sa-%s-%s-%s-%s", "azureintegration", var.env, "sea", var.instance)
