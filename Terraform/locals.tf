locals {
  ...
  # Resource Group Name
  resource_group_name = format("rg-%s-%s-%s-%s", "azint", var.env, "sea", var.instance) // rg-azint-dev-sea-01

  # Key Vault
  keyvault_name = format("kv-%s-%s-%s-%s", "azint", var.env, "sea", var.instance)

  # Client Storage Account Name
  storage_account_name       = format("sa%s%s%s%s", "azint", var.env, "sea", var.instance)            // saazintdevsea01
