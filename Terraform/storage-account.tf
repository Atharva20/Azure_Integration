...
resource "azurerm_storage_container" "inputshipmetjsons" {
  count                 = var.deploy_tier ? 1 : 0
  name                  = "inputshipmetjsons"
  storage_account_name  = azurerm_storage_account.clientstorageaccount[0].name
  container_access_type = var.container_access_type
}

resource "azurerm_storage_container" "outputshipmetcsv" {
  count                 = var.deploy_tier ? 1 : 0
  name                  = "outputshipmetcsv"
  storage_account_name  = azurerm_storage_account.clientstorageaccount[0].name
  container_access_type = var.container_access_type
}
...
resource "azurerm_storage_blob" "example" {
  name                   = "example.txt"
  storage_account_name   = azurerm_storage_account.clientstorageaccount[0].name
  storage_container_name = azurerm_storage_container.inputshipmetjsons[0].name
  type                   = "Block"
