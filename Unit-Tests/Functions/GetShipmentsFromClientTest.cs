namespace AzureIntegration.UnitTests
{
    ...
    public class GetShipmentsFromClientTest : UnitTestHelper
    {
        ...
        public async Task GetShipmentsFromClient_ShipmentDoesNotExists_ReturnsBadRequest() //methodname_scenario_expectation
        {
            ...
            var result = await getShipmentsFromClient.Run(new TimerInfo(null, null));
            var badResultMsg = (BadRequestObjectResult)result;

            //Assert
            Assert.IsType<BadRequestObjectResult>(result);
            Assert.Contains("Empty request body", Convert.ToString(badResultMsg.Value));
        ...
        public async Task GetShipmentsFromClient_ShipmentExists_ReturnsShipmentDetailsResponse() //methodname_scenario_expectation
        {
            ...
            var shipmentResponse = (ShipmentResponse)okResult.Value;

            //Assert
            Assert.IsType<OkObjectResult>(result);
            Assert.IsType<ShipmentResponse>(okResult.Value);
            Assert.Contains($"transformed-shipment-csv/{currentDateTime}", shipmentResponse.OutputTransformedLoc);
            Assert.Contains("3", Convert.ToString(shipmentResponse.TotalShipmentsReceieved));
            Assert.Contains("2", Convert.ToString(shipmentResponse.TotalShipmentsTransformed));
        }
    }
}
